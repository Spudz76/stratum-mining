...
import lib.logger
log = lib.logger.get_logger('sia_rpc_manager')

from lib.sia_rpc import SiaRPC
...
class SiaRPCManager(object):
    ...
    def __init__(self):
        ...
        self.conns = {}
        self.conns[0] = SiaRPC(settings.COINDAEMON_TRUSTED_HOST,
        ...
        for x in range (1, 99):
            if hasattr(settings, 'COINDAEMON_TRUSTED_HOST_' + str(x)) and hasattr(settings, 'COINDAEMON_TRUSTED_PORT_' + str(x)) and hasattr(settings, 'COINDAEMON_TRUSTED_USER_' + str(x)) and hasattr(settings, 'COINDAEMON_TRUSTED_PASSWORD_' + str(x)):
                self.conns[len(self.conns)] = SiaRPC(settings.__dict__['COINDAEMON_TRUSTED_HOST_' + str(x)],
    ...
    def add_connection(self, host, port, user, password):
        ...
        self.conns[len(self.conns)] = SiaRPC(host, port, user, password)
