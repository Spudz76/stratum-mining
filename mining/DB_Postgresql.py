...
class DB_Postgresql():
    ...
    def archive_found(self,found_time):
        self.dbc.execute(
            'insert into shares_archive_found select * from shares where upstream_result = true and time <= to_timestamp(%s)', [found_time])
        self.dbh.commit()
...
#        data: [worker_name,block_header,block_hash,difficulty,timestamp,is_valid,ip,block_height,prev_hash,invalid_reason,best_diff]
    ...
    def update_version_6(self):
        ...
        self.dbc.execute(
            'UPDATE pool_worker SET password = encode(digest(concat(password, %s), \'sha1\'), \'hex\') WHERE id > 0', [self.salt])
        self.dbh.commit()
